...
两者都是处理分支合并的命令，区别在于`merge`是将两个分支合并到当前分支，而`rebase`是将当前分支合并到另一个分支
- `git merge`是分支结构，多人的提交历史以分支的形式保留
- `git rebase`是线性结构，多人的提交历史以线性形式保留（后提交内容被复写并修改到前一次的提交中）
- `git commit -m --amend`：修改最后一次提交内容
