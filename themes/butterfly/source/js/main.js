document.addEventListener("DOMContentLoaded", () => {
	...
	const addHighlightTool = () => {
		...
		const highlightMacStyleEle =
			'<div class="macStyle"><div class="mac-close"></div><div class="mac-minimize"></div><div class="mac-maximize"></div></div>';
		...
		const highlightToolsFn = (e) => {
			...
			if ($target.contains("expand")) highlightShrinkFn(currentElement);
				...
				highlightCopyFn(currentElement, e.target);
			else if ($target.contains("fullpage-button"))
				codeFullpage(currentElement, e.target);
		};
		...
		const getActualHeight = (item) => {
			...
			const fix = () => {
				...
				hiddenElements.forEach((originalStyle, elem) => {
					elem.setAttribute(
						"style",
						originalStyle ? originalStyle + ";" + style : style
					);
				});
			};
		...
		const createEle = (lang, item) => {
			...
			if (
				highlightHeightLimit &&
				getActualHeight(item) > highlightHeightLimit + 30
			) {
	...
	const addJustifiedGallery = async (ele, tabs = false) => {
		...
		const init = async () => {
			for (const item of ele) {
				...
				try {
					const content =
						item.getAttribute("data-type") === "url"
							? await fetchUrl(text)
							: JSON.parse(text);
	...
	const rightSideFn = {
		...
		"mobile-toc-button": (p, item) => {
			...
			if (tocEleHeight > tocEleBottom) {
				tocEle.style.transformOrigin = `right ${
					tocEleHeight - tocEleBottom - btData.height / 2
				}px`;
			}
	...
	const openMobileMenu = () => {
		...
		btf.addEventListenerPjax(toggleMenu, "click", () => {
			sidebarFn.open();
		});
	};
		...
		hideButtons.forEach((item) =>
			item.addEventListener(
				...
				(e) => {
					...
					addJustifiedGallery(
						currentTarget.nextElementSibling.querySelectorAll(
							".gallery-container"
						)
					);
				},
	...
	const tabsFn = () => {
		const navTabsElements = document.querySelectorAll(
		...
		const handleNavClick = (e) => {
			...
			addJustifiedGallery(
				tabContents[indexOfButton].querySelectorAll(".gallery-container"),
				true
			);
		};
		...
		navTabsElements.forEach((tabElement) => {
			...
			btf.addEventListenerPjax(
				tabElement.lastElementChild,
				"click",
				handleToTopClick(tabElement)
			);
		});
	};
	...
	const addPostOutdateNotice = () => {
		const { limitDay, messagePrev, messageNext, position } =
			GLOBAL_CONFIG.noticeOutdate;
	...
	const justifiedIndexPostUI = () => {
		...
		if (
			!(recentPostsElement && recentPostsElement.classList.contains("masonry"))
		)
			return;

		const init = () => {
			const masonryItem = new InfiniteGrid.MasonryInfiniteGrid(
				".recent-post-items",
				{
			...
			btf.addGlobalFn(
				...
				"removeJustifiedIndexPostUI"
			);
		};

		typeof InfiniteGrid === "function"
			? init()
	...
	const forPostFn = () => {
		...
		addPhotoFigcaption();
		addJustifiedGallery(
			document.querySelectorAll("#article-container .gallery-container")
		);
		runLightbox();
		scrollFnToDo();
		addTableWrap();
